#!/usr/bin/env bash
#
# Lance les neuf bancs d'essai de la réception des demandes.
#
#   ./run-all.sh
#
...
titre "1/9 — Défenses : jeton, pot de miel, limitation de débit"
"$PHP_BIN" test-security.php
verdict $? "test-security.php"

titre "2/9 — Conservation : type de contenu privé et repository"
"$PHP_BIN" test-storage.php
verdict $? "test-storage.php"

titre "3/9 — Contrôleur de soumission"
"$PHP_BIN" test-controller.php
verdict $? "test-controller.php"

titre "4/9 — Rétention à 365 jours"
"$PHP_BIN" test-retention.php
verdict $? "test-retention.php"

titre "5/9 — Concurrence, atomicité et planification"
"$PHP_BIN" test-concurrence.php
verdict $? "test-concurrence.php"

titre "6/9 — Registre des références et verrou de programmation"
"$PHP_BIN" test-registre.php
verdict $? "test-registre.php"

titre "7/9 — Politique, normalisation et stockage des documents"
"$PHP_BIN" test-documents.php
verdict $? "test-documents.php"

titre "8/9 — Transaction, liens signés et rétention des documents"
"$PHP_BIN" test-transaction.php
verdict $? "test-transaction.php"

titre "9/9 — Compatibilité et absence d'effet public"
