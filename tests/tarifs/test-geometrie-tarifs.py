#!/usr/bin/env python3
"""Banc de géométrie de la page Tarifs — mesure réelle, dans un vrai moteur.
...
silencieux.
    ...
    python3 test-geometrie-tarifs.py
"""
...
APERCU = os.path.join(RACINE, "tests", "tarifs", "apercu-tarifs.html")
GENERATEUR = os.path.join(RACINE, "tests", "tarifs", "apercu-tarifs.php")
...
def regenerer():
    ...
    with open(APERCU, "w", encoding="utf-8") as sortie:
        code = subprocess.call([php, GENERATEUR], stdout=sortie)

    if code != 0 or not os.path.getsize(APERCU):
        print("La génération de l'aperçu a échoué (code %s)." % code)
        sys.exit(1)


def mesurer(chrome, url):
    profil = tempfile.mkdtemp(prefix="urbizen-tarifs-")
...
def main():
    ...
    try:
        url = "http://127.0.0.1:%d/tests/tarifs/geometrie-tarifs.html" % port
